// ============================================================
    ...
    const LEVELS = [
      ...
      },
      {
        ...
        waves: 3,
        bg: '#2d5a27',
        description: "Whole lotta cover, watch out for tanks!"
      },
      {
        id: 3,
        name: "Towers",
        ...
        waves: 4,
        bg: '#5c4a3a',
        description: "Big but few obstacles, watch out for the drones."
      },
      {
        id: 4,
        name: "Overpowerd",
      ...
      _updateIdlePatrol() {
        ...
        if (d <= 6) {
          ...
          if (this.idleTimer > 45) {
            this.idleTimer = 0;
            const roamAngle = Math.random() * Math.PI * 2;
            const roamDist = 30 + Math.random() * (this.type === 'tank' ? 90 : this.type === 'sniper' ? 110 : 70);
            this.targetX = Math.max(this.radius, Math.min(canvas.width - this.radius, this.x + Math.cos(roamAngle) * roamDist));
            this.targetY = Math.max(this.radius, Math.min(canvas.height - this.radius, this.y + Math.sin(roamAngle) * roamDist));
          }
        } else {
    ...
    canvas.addEventListener('mouseleave', () => { game.builderHoverPos = null; });
    ...
    canvas.addEventListener('mouseup', e => {
      ...
      if (e.button === 0 && game.isSelecting) {
        ...
        else { s.textContent = 'NONE'; d.textContent = 'Drag to select units.'; }
      }
    });
    ...
    document.addEventListener('keydown', e => {
      ...
      // Builder shortcuts
      if (game.mode === 'builder') {
        const map = { '1': 'brush-ruins', '2': 'brush-bunker', '3': 'brush-wall', '4': 'brush-tree', '5': 'brush-eraser', '6': 'brush-spawn-inf', '7': 'brush-spawn-tank', '8': 'brush-spawn-drone', '9': 'brush-spawn-sniper', '0': 'brush-spawn-anti-tank', '-': 'brush-filled', '=': 'brush-wreck' };
    ...
    function buyUnit(type) {
      ...
      game.credits -= cost;
      const u = spawnUnit('blue', type, 70 + Math.random() * 70, 150 + Math.random() * (canvas.height - 300));
      updateTeamUiCounts();
      playSound('deploy');
    ...
    function updateRuinLabel() {
      if (game.activeBrush === 'wreck') {
        ...
        document.getElementById('ruins-size-label').textContent = `${w}×${h}`;
      }
    }
    ...
    function draw() {
      ...
      for (let i = 0; i <= canvas.width / GRID + 1; i++) { ctx.beginPath(); ctx.moveTo(i * GRID, 0); ctx.lineTo(i * GRID, canvas.height); ctx.stroke(); }
      for (let j = 0; j <= canvas.height / GRID + 1; j++) { ctx.beginPath(); ctx.moveTo(0, j * GRID); ctx.lineTo(canvas.width, j * GRID); ctx.stroke(); }
